...
class SingleScheduleHelper
{
    ...
    {
        $merchant = \fbwidget\models\MtMerchant::findOne($merchant_id);
        
        
        $s = \fbwidget\models\MerchantSchedule::find()->where(['work_date' => $d, 'merchant_id' => $merchant_id])->one();
        ...
        if ($s) {
            ...
            if ($merchant->lastSchedule && $merchant->lastSchedule->{strtolower(date('D', strtotime($d)))}) {
                $tr = \fbwidget\models\ScheduleDaysTemplate::findOne(['id'=>$merchant->lastSchedule->{strtolower(date('D', strtotime($d)))}]);
    ...
    public static function isStaffWork($d, $t, $m, $staff, $u, $c)
    {
        if ($staff->staffVacations) {
                ...
                $vacation = \fbwidget\models\StaffVacation::find()
                    ->where(['<=','start_date', $d ])
                    ->andWhere(['>=', 'end_date', $d])
        ...
        $s = \fbwidget\models\StaffSchedule::find()->where(['work_date' => $d, 'staff_id' => $staff->id])->one();
        ...
        if ($s) {
            ...
            if ($staff->lastSchedule && $staff->lastSchedule->{strtolower(date('D', strtotime($d)))}) {
                $tr = \fbwidget\models\ScheduleDaysTemplate::findOne($staff->lastSchedule->{strtolower(date('D', strtotime($d)))});
...
//        $crit->addCondition('order_time<"' . date('Y-m-d', strtotime($d)) . ' 23:59:00"');
        ...
        $orders = \fbwidget\models\Order::find()
